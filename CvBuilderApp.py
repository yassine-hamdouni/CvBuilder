...
.top-banner h1   { margin:0; font-size:22px; font-weight:700; color:#1e3a5f; letter-spacing:.5px; }
...
.stButton>button { background:#1e3a5f; color:#fff; border-radius:5px; border:none; font-weight:600; }
.stButton>button:hover { background:#e8251f; }
[data-testid="stExpander"] { border:1px solid #dde3ea; border-radius:6px; background:#fff; }
</style>
...
<div class="top-banner">
  {_img_tag}
  <div><h1>CV Builder</h1><p>Créez votre CV professionnel au format ExpertNow</p></div>
...
with st.sidebar:
    ...
    # ── JSON import ──────────────────────────────────────────────────────────
    # st.file_uploader is wiped by st.rerun(), so we parse in the SAME run
    # the file appears. A name+size guard prevents re-parsing on every rerun.
...
# Re-alias AFTER the sidebar (post-import rerun lands here)
...
with c3:
    # UX FIX: number_input instead of free-text string
...
for i, edu in enumerate(cv["education"]):
    with st.expander(f"Formation {i+1}", expanded=(i == 0)):
        ...
        if st.button("🗑️ Supprimer", key=f"re{i}"):
